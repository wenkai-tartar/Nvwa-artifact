...
usage() {
  cat <<'EOF'
Usage: bash scripts/run_lightweight_experiments.sh [EXPERIMENT|all ...]

Runs the lightweight versions of Experiments 1-9 sequentially. With no
...
LIGHTWEIGHT_SKIP_BUILD="${SKIP_BUILD:-1}"
...
LATEST_SUITE_POINTER="${AE_ROOT}/results/latest_lightweight_experiments.txt"
...
if [[ "${RESULTS_ROOT_WAS_SET}" == "0" ]]; then
  if [[ "${RERUN_ALL}" != "1" && "${RUN_ID_WAS_SET}" == "0" ]] &&
    ...
    RESULTS_ROOT="${AE_ROOT}/results/lightweight_experiments_${RUN_ID}"
  fi
fi
mkdir -p "${RESULTS_ROOT}"
RESULTS_ROOT="$(cd "${RESULTS_ROOT}" && pwd)"
RUN_LOG="${RESULTS_ROOT}/lightweight_experiments_${RUN_ID}.log"

exec > >(tee -a "${RUN_LOG}") 2>&1

mkdir -p "${AE_ROOT}/results"
printf '%s\n' "${RESULTS_ROOT}" > "${AE_ROOT}/results/latest_lightweight_experiments.txt"

echo "lightweight_experiments_run_id=${RUN_ID}"
echo "lightweight_experiments_results=${RESULTS_ROOT}"
echo "lightweight_experiments_data=$(suite_data_dir "${RESULTS_ROOT}")"
echo "lightweight_experiments_log=${RUN_LOG}"
...
run_step() {
  ...
  start_s="$(date +%s)"
  if (cd "${AE_ROOT}" && env AE_ROOT="${AE_ROOT}" RESULTS_ROOT="${RESULTS_ROOT}" RESUME_POLICY="${RESUME_POLICY}" "$@"); then
    local result_dir
  ...
  case "$1" in
    1)
      run_step 1 "FatTree Ring AllReduce" \
        ...
        RUN_RULEBASED=1 \
        REPEATS=1 \
        SKIP_BUILD="${LIGHTWEIGHT_SKIP_BUILD}" \
        bash "${SCRIPT_DIR}/run_experiment1_fattree_ring_allreduce.sh"
      ;;
    2)
      run_step 2 "FatTree Memory Profile" \
        GLOBAL_K_VALUES="4,8,16" \
        NODEBFS_K_VALUES="4,8,16" \
        RULEBASED_K_VALUES="4,8,16" \
        RUN_GLOBAL=0 \
        RUN_NODEBFS=1 \
        RUN_RULEBASED=0 \
        REPEATS=1 \
        SKIP_BUILD="${LIGHTWEIGHT_SKIP_BUILD}" \
        bash "${SCRIPT_DIR}/run_experiment4_fattree_memory_profile.sh" --routings NodeBfs
      ;;
    3)
...
echo
echo "lightweight_experiments_status=passed"
echo "lightweight_experiments_results=${RESULTS_ROOT}"
echo "lightweight_experiments_data=$(suite_data_dir "${RESULTS_ROOT}")"
echo "lightweight_experiments_ran=${ran_experiments[*]:-none}"
echo "lightweight_experiments_skipped=${skipped_experiments[*]:-none}"
if [[ "${PLOT_AFTER}" == "1" ]]; then
  echo "lightweight_experiments_figures=${RESULTS_ROOT}/figures"
fi
