...
ae_init_results_dir "experiment3_torus_ring_allreduce" "${AE_ROOT}"
ae_start_run_log "experiment3_torus_ring_allreduce"
ae_print_run_header
ae_install_signal_traps

TORUS_D_VALUES_WAS_SET="${TORUS_D_VALUES+x}"
TORUS_D_VALUES="${TORUS_D_VALUES:-5,10,15,20,25,30}"
GLOBAL_D_VALUES="${GLOBAL_D_VALUES:-}"
if [[ -z "${GLOBAL_D_VALUES}" ]]; then
  if [[ -n "${TORUS_D_VALUES_WAS_SET}" ]]; then
    GLOBAL_D_VALUES="${TORUS_D_VALUES}"
  else
    GLOBAL_D_VALUES="5,10"
  fi
fi
NODEBFS_D_VALUES="${NODEBFS_D_VALUES:-${TORUS_D_VALUES}}"
RULEBASED_D_VALUES="${RULEBASED_D_VALUES:-${TORUS_D_VALUES}}"
TRAFFIC_PATTERN="${TRAFFIC_PATTERN:-grouped-allreduce}"
DATA_SIZE="${DATA_SIZE:-1048576}"
if [[ "${THREADS:-1}" != "1" ]]; then
  echo "[info] Experiment 3 is single-threaded; ignoring THREADS=${THREADS} and using THREADS=1"
fi
THREADS=1
...
TORUS_NODEBFS_ROUTING="${TORUS_NODEBFS_ROUTING:-NodeBfsWithHost}"
...
if [[ "${RUN_GLOBAL}" == "1" ]]; then
  ROUTING_VALUES+=(--routing-values "Global:${GLOBAL_D_VALUES}")
fi
if [[ "${RUN_NODEBFS}" == "1" ]]; then
  ROUTING_VALUES+=(--routing-values "${TORUS_NODEBFS_ROUTING}:${NODEBFS_D_VALUES}")
fi
if [[ "${RUN_RULEBASED}" == "1" ]]; then
  ROUTING_VALUES+=(--routing-values "RuleBased:${RULEBASED_D_VALUES}")
fi
...
ae_run python3 "${SCRIPT_DIR}/run_topology_ring_allreduce_stats.py" \
  ...
  --out-dir "${RESULTS_DIR}" \
  --stats-name "experiment_5.csv" \
  --runs-name "experiment_5_runs.csv" \
  "${EXTRA_ARGS[@]}"

mkdir -p "${AE_ROOT}/results"
printf '%s\n' "${RESULTS_DIR}" > "${AE_ROOT}/results/latest_torus_ring_allreduce.txt"

cat <<EOF
experiment3_results=${RESULTS_DIR}
experiment_5_stats=${RESULTS_DIR}/experiment_5.csv
EOF
