...
def save_legend_only(filename):
    ...
    fig, ax = plt.subplots(figsize=(6.9, 0.5))
    ax.axis("off")
    ax.legend(
        handles=handles,
        loc="center",
...
save_legend_only(output_path("legend1.pdf"))
...
def dedup_legend(ax, loc="upper center", ncol=3, **kwargs):
    ...
    leg = ax.legend(
        ...
        loc=loc,
        bbox_to_anchor=(0.8, 1.22),  # ✅ 图外上方（数值可微调：1.15~1.35）
        ncol=ncol,                   # ✅ 横着排
        columnspacing=1.2,
        handlelength=2.8,
        handletextpad=0.8,
...
def plot_metric(df_sub, topo, metric, ylabel, filename, logy=True, kind="line", ylim=None, xlim=None):
    ...
    dedup_legend(ax, loc="best", fontsize=15)
...
for topo in ["fattree", "dragonfly", "torus"]:
    ...
    if topo == "fattree":
        plot_metric(
            ...
            filename=output_path(f"init_{topo}_legend.pdf"),
            logy=True,
            kind="line",
            ylim=(1e-2, 1e6),
            xlim=(0, 11) 
