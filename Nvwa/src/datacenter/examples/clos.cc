...
NS_LOG_COMPONENT_DEFINE("ClosExample");
...
main(int argc, char* argv[])
{
    ...
    // Configure logging
    LogComponentEnable("ClosExample", LOG_LEVEL_INFO);

    std::filesystem::create_directory("./src/datacenter/examples/traces");
    // Open packet trace file
    packetTraceFile.open("./src/datacenter/examples/traces/clos_packet_trace.txt");
    if (!packetTraceFile.is_open())
    {
    ...
    if (packetTraceFile.is_open())
    {
        ...
        std::cout << "Packet trace saved to: "
                     "./src/datacenter/examples/traces/clos_packet_trace.txt"
                  << std::endl;
    }
