...
#include "dragonfly-ugal-routing.h"
...
NS_LOG_COMPONENT_DEFINE("DragonflyUgalRouting");

NS_OBJECT_ENSURE_REGISTERED(DragonflyUgalRouting);
...
TypeId
DragonflyUgalRouting::GetTypeId()
{
    static TypeId tid =
        TypeId("ns3::DragonflyUgalRouting")
            .SetParent<Ipv4RoutingProtocol>()
            .SetGroupName("Datacenter")
            .AddConstructor<DragonflyUgalRouting>()
            .AddAttribute("RouterLevel",
                          "Level id for router nodes in StructuredTopology.",
                          UintegerValue(1),
                          MakeUintegerAccessor(&DragonflyUgalRouting::SetRouterLevel),
                          MakeUintegerChecker<uint32_t>())
            .AddAttribute("GroupDimId",
                          "Dimension id encoding group ID in StructuredAddress.",
                          UintegerValue(1),
                          MakeUintegerAccessor(&DragonflyUgalRouting::SetGroupDimId),
                          MakeUintegerChecker<uint32_t>())
            .AddAttribute("Seed",
                          "Seed for deterministic transit group selection.",
                          UintegerValue(1),
                          MakeUintegerAccessor(&DragonflyUgalRouting::SetSeed),
                          MakeUintegerChecker<uint64_t>())
            .AddAttribute("Alpha",
                          "UGAL alpha coefficient.",
                          DoubleValue(1.0),
                          MakeDoubleAccessor(&DragonflyUgalRouting::SetAlpha,
                                             &DragonflyUgalRouting::GetAlpha),
                          MakeDoubleChecker<double>())
            .AddAttribute("DetourPenalty",
                          "UGAL detour penalty.",
                          DoubleValue(1.0),
                          MakeDoubleAccessor(&DragonflyUgalRouting::SetDetourPenalty,
                                             &DragonflyUgalRouting::GetDetourPenalty),
                          MakeDoubleChecker<double>())
            .AddAttribute("QueueMetric",
                          "Congestion metric for UGAL: none, packets, or bytes.",
                          EnumValue(QueueMetric::kBytes),
                          MakeEnumAccessor<QueueMetric>(&DragonflyUgalRouting::GetQueueMetric,
                                                        &DragonflyUgalRouting::SetQueueMetric),
                          MakeEnumChecker(QueueMetric::kNone,
                                          "none",
                                          QueueMetric::kPackets,
                                          "packets",
                                          QueueMetric::kBytes,
                                          "bytes"));
    return tid;
...
DragonflyUgalRouting::DragonflyUgalRouting()
    : m_ipv4(nullptr),
      m_node(nullptr),
      m_topo(nullptr)
{
    NS_LOG_FUNCTION(this);
    m_congestionProvider = std::make_shared<NetDeviceQueueSignalProvider>(m_queueMetric);
}

DragonflyUgalRouting::DragonflyUgalRouting(Ptr<Node> node, Ptr<StructuredTopology> topo)
    : m_ipv4(nullptr),
      m_node(node),
      m_topo(topo)
{
...
void
DragonflyUgalRouting::SetTopology(Ptr<StructuredTopology> topo)
{
...
void
DragonflyUgalRouting::SetRouterLevel(uint32_t level)
{
    m_routerLevel = level;
    m_cacheBuilt = false;
}
...
void
DragonflyUgalRouting::SetGroupDimId(uint32_t dimId)
{
    m_groupDimId = dimId;
    m_cacheBuilt = false;
}
...
void
DragonflyUgalRouting::SetSeed(uint64_t seed)
{
    m_seed = seed;
}

void
DragonflyUgalRouting::SetAlpha(double alpha)
{
    m_alpha = alpha;
}

double
DragonflyUgalRouting::GetAlpha() const
{
    return m_alpha;
}

void
DragonflyUgalRouting::SetDetourPenalty(double penalty)
{
    m_detourPenalty = penalty;
}

double
DragonflyUgalRouting::GetDetourPenalty() const
{
    return m_detourPenalty;
}

void
DragonflyUgalRouting::SetQueueMetric(QueueMetric metric)
{
    m_queueMetric = metric;
    auto provider = std::dynamic_pointer_cast<NetDeviceQueueSignalProvider>(m_congestionProvider);
    if (provider)
    {
...
QueueMetric
DragonflyUgalRouting::GetQueueMetric() const
{
    return m_queueMetric;
}

void
DragonflyUgalRouting::DoDispose()
{
    NS_LOG_FUNCTION(this);
    m_topo = nullptr;
    m_ipv4 = nullptr;
    m_node = nullptr;
    m_congestionProvider.reset();
    Ipv4RoutingProtocol::DoDispose();
}

void
DragonflyUgalRouting::SetIpv4(Ptr<Ipv4> ipv4)
{
...
void
DragonflyUgalRouting::NotifyInterfaceUp(uint32_t interface)
{
...
void
DragonflyUgalRouting::NotifyInterfaceDown(uint32_t interface)
{
    NS_LOG_FUNCTION(this << interface);
    m_cacheBuilt = false;
}
...
void
DragonflyUgalRouting::NotifyAddAddress(uint32_t interface, Ipv4InterfaceAddress address)
{
...
void
DragonflyUgalRouting::NotifyRemoveAddress(uint32_t interface, Ipv4InterfaceAddress address)
{
    NS_LOG_FUNCTION(this << interface << address);
    m_cacheBuilt = false;
}
...
void
DragonflyUgalRouting::PrintRoutingTable(Ptr<OutputStreamWrapper> stream, Time::Unit unit) const
{
    (void)stream;
    (void)unit;
}
...
void
DragonflyUgalRouting::SetTraceRouting(bool enable)
{
    m_traceRouting = enable;
}

void
DragonflyUgalRouting::SetTraceStream(std::ostream* os)
{
    m_traceStream = os;
}

void
DragonflyUgalRouting::SetTraceVerbose(bool enable)
{
    m_traceVerbose = enable;
}

void
DragonflyUgalRouting::DumpCaches(std::ostream& os)
{
    if (!EnsureCaches())
    {
        os << "DragonflyUgalRouting: cache not ready" << std::endl;
        return;
    }
    ...
    os << "\n=== DragonflyUgalRouting cache dump (node " << m_nodeId << ") ==="
       << std::endl;
...
bool
DragonflyUgalRouting::EnsureCaches()
{
    if (m_cacheBuilt)
    {
        return true;
...
uint64_t
DragonflyUgalRouting::ComputeFlowHash(const Ipv4Header& h,
                                         Ptr<const Packet> p,
                                         uint32_t seed) const
{
    union
...
uint32_t
DragonflyUgalRouting::GetGroupFromAddr(const StructuredAddress& addr) const
{
...
uint32_t
DragonflyUgalRouting::GetNodeIdForIp(Ipv4Address ip) const
{
...
bool
DragonflyUgalRouting::IsHost(uint32_t nodeId) const
{
    return m_hostNodes.find(nodeId) != m_hostNodes.end();
}

bool
DragonflyUgalRouting::IsRouter(uint32_t nodeId) const
{
    return m_routerNodes.find(nodeId) != m_routerNodes.end();
}

std::optional<uint32_t>
DragonflyUgalRouting::FindNextHopToRouter(uint32_t srcRouterId,
...
std::optional<uint32_t>
DragonflyUgalRouting::FindNextHopToGroup(uint32_t srcRouterId,
...
std::optional<DragonflyUgalRouting::NeighborInfo>
DragonflyUgalRouting::GetNeighbor(uint32_t nodeId, uint32_t nextNodeId) const
{
...
std::optional<uint32_t>
DragonflyUgalRouting::GetGlobalNeighbor(uint32_t nodeId, uint32_t targetGroup) const
{
...
uint32_t
DragonflyUgalRouting::PickValiantGroup(uint32_t srcGroup,
                                          uint32_t dstGroup,
                                          uint64_t hash) const
{
...
std::optional<DragonflyUgalRouting::NextHop>
DragonflyUgalRouting::LookupNextHop(const Ipv4Header& header,
                                       Ptr<const Packet> p,
                                       bool isInject) const
{
    ...
    auto traceDecision = [&](const std::string& action,
        ...
        std::ostringstream oss;
        oss << Simulator::Now().GetSeconds() << "\tDUR\t"
            << "node=" << m_nodeId << "\t"
            << "inject=" << (isInject ? 1 : 0) << "\t"
    ...
    if (tag.GetAlgorithm() == NonMinimalAlgorithm::kUgal &&
        tag.GetPhase() == NonMinimalPhase::kMin &&
        tag.GetFlags() == kUgalPending &&
        tag.HasRewrites() &&
        groupFieldIndex.has_value())
    {
        for (const auto& entry : tag.GetRewrites())
        {
            if (entry.index == *groupFieldIndex)
            {
                midGroup = entry.value;
                midGroupOpt = midGroup;
                break;
            }
        }
    ...
    const bool hasPendingTransit =
        tag.GetAlgorithm() == NonMinimalAlgorithm::kUgal &&
        tag.GetPhase() == NonMinimalPhase::kMin &&
        tag.GetFlags() == kUgalPending &&
        tag.HasRewrites();
    auto updateTagStr = [&]() {
        ...
        oss << static_cast<int>(tag.GetAlgorithm()) << "/"
            << static_cast<int>(tag.GetPhase()) << "/f=" << static_cast<int>(tag.GetFlags());
        if (tag.HasRewrites())
        {
            oss << "/rw=[";
    ...
    auto computeNextHop =
        [&](uint32_t targetGroup) -> std::optional<NextHop> {
        if (curIsHost)
        {
            return hostNextHop;
        }
    ...
    auto probeMetric =
        [&](uint32_t targetGroup) -> std::pair<std::optional<NextHop>, uint64_t> {
        std::pair<std::optional<NextHop>, uint64_t> res;
        res.first = computeNextHop(targetGroup);
        res.second = 0;
        if (res.first.has_value() && m_congestionProvider && m_ipv4 &&
            res.first->ifIndex < m_ipv4->GetNInterfaces())
        {
            res.second =
                m_congestionProvider->GetMetric(m_ipv4->GetNetDevice(res.first->ifIndex));
        }
        return res;
    };

    uint32_t srcNodeId = GetNodeIdForIp(header.GetSource());
    bool isSourceRouter = false;
    if (srcNodeId != UINT32_MAX)
    {
        if (IsRouter(srcNodeId) && srcNodeId == m_nodeId)
        {
            isSourceRouter = true;
        }
        auto itSrcRouter = m_hostToRouter.find(srcNodeId);
        if (itSrcRouter != m_hostToRouter.end() && itSrcRouter->second == m_nodeId)
        {
            isSourceRouter = true;
        }
    }
    const bool shouldDecide = (isSourceRouter && hasPendingTransit) ||
                              (isInject && IsRouter(m_nodeId));

    if (isInject && !shouldDecide && p && groupFieldIndex.has_value() && midGroup != 0 &&
        dstGroup != 0)
    {
        NonMinimalRoutingTag nextTag = tag;
    ...
    else if (shouldDecide && p && groupFieldIndex.has_value() && midGroup != 0 && dstGroup != 0)
    {
        auto minProbe = probeMetric(dstGroup);
        auto valProbe = probeMetric(midGroup);

        double minCost = static_cast<double>(minProbe.second);
        double valCost =
            static_cast<double>(valProbe.second) + m_alpha * m_detourPenalty;

        bool pickVal = false;
        if (valProbe.first.has_value() && !minProbe.first.has_value())
        {
            pickVal = true;
        }
        else if (!valProbe.first.has_value())
        {
            pickVal = false;
        }
        else
        ...
        else if (tag.GetAlgorithm() != NonMinimalAlgorithm::kUgal ||
                 tag.GetPhase() != NonMinimalPhase::kMin || tag.GetFlags() != 0 ||
                 tag.HasRewrites())
        {
            NonMinimalRoutingTag nextTag = tag;
            nextTag.SetAlgorithm(NonMinimalAlgorithm::kUgal);
...
Ptr<Ipv4Route>
DragonflyUgalRouting::RouteOutput(Ptr<Packet> p,
...
bool
DragonflyUgalRouting::RouteInput(Ptr<const Packet> p,
                                    const Ipv4Header& header,
...
void
DragonflyUgalRouting::BuildCaches()
{
    m_cacheBuilt = false;
    m_nodeLevel.clear();
    m_nodeAddr.clear();
